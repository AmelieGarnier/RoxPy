...
// Test de placement — 12 PALIERS de difficulté croissante.
// À chaque test, une question est tirée au hasard dans chaque palier (et
// l'ordre des réponses est mélangé côté jeu), donc le test change à chaque fois.
// Le score (0-12) reste comparable → zone de départ via zoneDepart().
export const PLACEMENT = [
  // palier 1 — print / bases
  [
    {q:"Que fait print(\"Salut\") ?", opts:["Affiche Salut","Crée une variable Salut","Rien"], a:0},
    {q:"Comment afficher le texte Bonjour à l'écran ?", opts:["print(\"Bonjour\")","echo Bonjour","affiche(\"Bonjour\")"], a:0},
    {q:"Que met-on autour d'un texte (chaîne) en Python ?", opts:["des guillemets \"...\"","des crochets [...]","rien"], a:0},
  ],
  // palier 2 — variables
  [
    {q:"Après x = 5 puis x = x + 2, que vaut x ?", opts:["5","7","x + 2"], a:1},
    {q:"Après a = 3 et b = a, que vaut b ?", opts:["3","0","a"], a:0},
    {q:"Quelle ligne crée une variable âge valant 10 ?", opts:["age = 10","10 = age","age == 10"], a:0},
  ],
  // palier 3 — opérateurs / nombres
  [
    {q:"Que vaut 7 % 2 ?", opts:["3.5","1","3"], a:1},
    {q:"Que vaut 2 ** 3 ?", opts:["6","8","9"], a:1},
    {q:"Que vaut 7 // 2 ?", opts:["3","3.5","4"], a:0},
  ],
  // palier 4 — chaînes
  [
    {q:"Que vaut len(\"chat\") ?", opts:["3","4","5"], a:1},
    {q:"Que vaut \"ab\" + \"cd\" ?", opts:["\"abcd\"","\"ab cd\"","une erreur"], a:0},
    {q:"Que vaut \"Roxy\".upper() ?", opts:["\"roxy\"","\"ROXY\"","\"Roxy\""], a:1},
  ],
  // palier 5 — booléens
  [
    {q:"Que vaut (3 > 1) and (1 > 3) ?", opts:["True","False","None"], a:1},
    {q:"Que vaut not True ?", opts:["True","False","0"], a:1},
    {q:"Que vaut (5 == 5) or False ?", opts:["True","False","5"], a:0},
  ],
  // palier 6 — listes / index
  [
    {q:"Que vaut [10, 20, 30][1] ?", opts:["10","20","30"], a:1},
    {q:"Que vaut [\"a\",\"b\",\"c\"][-1] ?", opts:["\"a\"","\"b\"","\"c\""], a:2},
    {q:"Que vaut len([4, 5, 6]) ?", opts:["2","3","6"], a:1},
  ],
  // palier 7 — boucles
  [
    {q:"Que génère range(4) ?", opts:["1,2,3,4","0,1,2,3","0,1,2,3,4"], a:1},
    {q:"Combien de tours fait for i in range(3) ?", opts:["2","3","4"], a:1},
    {q:"range(2, 5) génère…", opts:["2,3,4","2,3,4,5","3,4,5"], a:0},
  ],
  // palier 8 — dictionnaires
  [
    {q:"Comment accéder à la valeur de la clé \"a\" dans un dict d ?", opts:["d.a","d[\"a\"]","d(a)"], a:1},
    {q:"{\"x\": 1}[\"x\"] vaut…", opts:["\"x\"","1","une erreur"], a:1},
    {q:"Comment ajouter la clé \"n\" valant 5 au dict d ?", opts:["d[\"n\"] = 5","d.add(\"n\", 5)","d + {\"n\":5}"], a:0},
  ],
  // palier 9 — compréhensions
  [
    {q:"Que produit [n * 2 for n in [1, 2]] ?", opts:["[2, 4]","[1, 2, 1, 2]","une erreur"], a:0},
    {q:"Que produit [x for x in range(3)] ?", opts:["[0, 1, 2]","[1, 2, 3]","[0, 1, 2, 3]"], a:0},
    {q:"[c.upper() for c in \"ab\"] donne…", opts:["[\"A\", \"B\"]","\"AB\"","[\"ab\"]"], a:0},
  ],
  // palier 10 — fonctions
  [
    {q:"Que renvoie une fonction sans return ?", opts:["0","None","\"\""], a:1},
    {q:"Quel mot-clé définit une fonction ?", opts:["func","def","function"], a:1},
    {q:"def f(x): return x*2  →  f(4) vaut…", opts:["8","4","x*2"], a:0},
  ],
  // palier 11 — POO
  [
    {q:"Dans une classe, que désigne self ?", opts:["La classe","L'objet en cours","Le module"], a:1},
    {q:"Quelle méthode s'exécute à la création d'un objet ?", opts:["__init__","__start__","create"], a:0},
    {q:"Quel mot-clé crée une classe ?", opts:["class","object","struct"], a:0},
  ],
  // palier 12 — avancé
  [
    {q:"Quel mot-clé crée un générateur ?", opts:["return","yield","gen"], a:1},
    {q:"try / except sert à…", opts:["gérer les erreurs","faire une boucle","créer une classe"], a:0},
    {q:"Une fonction récursive est une fonction qui…", opts:["s'appelle elle-même","ne renvoie rien","boucle à l'infini"], a:0},
  ],
];
