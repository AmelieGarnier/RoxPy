...
<video src="https://github.com/user-attachments/assets/73e56455-cbe9-4384-9391-0f80e4930e5e" controls></video>
...
4. **Accès libre** : tu peux ouvrir n’importe quelle zone, même sans avoir terminé les précédentes — un message t’avertit simplement que les leçons d’avant t’aideraient.
...
## 📚 Les 35 zones en détail

### ✨ Forêt enchantée — Les bases (zones 1-7)
| # | Zone | Thème |
|---|------|-------|
| 1 | 🌱 Premiers pas | `print()`, chaînes, commentaires |
| 2 | 📦 Variables | affectation, noms de variables, échange |
| 3 | 🔢 Nombres & opérateurs | `+ - * / // % **`, `round()`, `abs()` |
| 4 | 🔤 Chaînes de caractères | concaténation, `len()`, `.upper()`/`.lower()` |
| 5 | ⌨️ input() & f-strings | saisie clavier, conversion, formatage `{x:.2f}` |
| 6 | ⚖️ Booléens & comparaisons | `True/False`, `== != < >`, `and/or/not` |
| 7 | 🔀 Conditions | `if / elif / else`, indentation |

### 🏔️ Montagne enneigée — Défis & logique (zones 8-15)
| # | Zone | Thème |
|---|------|-------|
| 8 | 📋 Listes | créer, indexer, `.append()`, `len()` |
| 9 | 🔪 Slicing & indices | `liste[a:b]`, bornes, pas négatif |
| 10 | 🔁 Boucle for | `range()`, parcours de séquence |
| 11 | 🤝 enumerate & zip | index+valeur, parcours parallèle |
| 12 | ⏳ Boucle while | répéter tant que, éviter la boucle infinie |
| 13 | 🎯 Tuples & sets | tuples non modifiables, déballage, ensembles |
| 14 | 📖 Dictionnaires | clés/valeurs, ajout, parcours |
| 15 | ✨ Compréhensions | `[expr for x in seq if ...]` |

### 🌙 Temple lunaire — Réflexion & mémoire (zones 16-22)
| # | Zone | Thème |
|---|------|-------|
| 16 | 🧩 Fonctions | `def`, paramètres, `return` |
| 17 | 🔭 Portée des variables | local vs global |
| 18 | 🎒 Arguments avancés | valeurs par défaut, arguments nommés |
| 19 | ⚡ Lambda, map & filter | mini-fonctions, transformer/filtrer |
| 20 | 📚 Modules & imports | `import`, `from ... import` |
| 21 | 🛡️ Gestion des erreurs | `try / except`, exceptions |
| 22 | 🪄 Chaînes avancées | `.split()`, `.join()`, `.replace()`, `.strip()` |

### 🏘️ Village forestier — Créativité & expression (zones 23-28)
| # | Zone | Thème |
|---|------|-------|
| 23 | 🏗️ POO : classes | `class`, `__init__`, `self`, méthodes |
| 24 | 🧬 POO : héritage | classe parente/enfant, surcharge |
| 25 | 📄 Fichiers | `open()`, `with`, lire/écrire |
| 26 | 🗂️ JSON | `json.dumps()`, `json.loads()` |
| 27 | 🕐 Dates & heures | module `datetime` |
| 28 | 🎲 Math & random | modules `math` et `random` |

### 🔮 Sanctuaire RoxPy — Maîtrise ultime (zones 29-35)
| # | Zone | Thème |
|---|------|-------|
| 29 | 🔍 Expressions régulières | module `re`, `findall` |
| 30 | 📊 Manipulation de données | filtrer, trier, agréger |
| 31 | 🧠 Algorithmes classiques | recherche linéaire/binaire, tri |
| 32 | 🪞 Récursivité | cas de base + appel récursif |
| 33 | 🌀 Générateurs | `yield`, valeurs à la demande |
| 34 | 🎮 Projet : le pendu | assembler un vrai mini-jeu |
| 35 | 👑 Le Grand Examen | boss final : tout combiné |


## 🙏 Crédits

- **Histoire, niveaux & pédagogie** : Amélie
- **Illustrations & cinématiques** : ChatGPT2 - Higgsfield & Google Flow
- **Progression des 35 zones** : inspirée du parcours **[30 Days Of Python](https://github.com/Asabeneh/30-Days-Of-Python)** d'Asabeneh, réadapté en aventure ludique.
- **Vibe codé avec Claude Code & Design**
